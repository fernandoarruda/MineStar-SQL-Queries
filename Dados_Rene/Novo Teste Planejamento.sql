...
OrigemBlock as 'OrigemBloco', 
Hierarchy_Block as 'HierarquiaBloco',
Destino,
Material_Operator,
Material_Block,
Material_MES,
...
round(sum(payload),4) as 'MassaTotal', --sum(Nota_CU) as 'SumNotaCU', 
round( (sum(Nota_CU) / sum(payload)) ,4) as 'MedPd_CU', --round(avg(CU),4) as 'MedSp_CU', sum(Nota_AU) as 'SumNotaAU', 
round( (sum(Nota_AU) / sum(payload)) ,4) as 'MedPd_AU', --round(avg(AU),4) as 'MedSp_AU',
...
round( (sum(Nota_BTOF) / sum(payload)) ,4) as 'MedPd_BTOF',
round( (sum(Nota_BTOS) / sum(payload)) ,4) as 'MedPd_BTOS', --round(avg(BTOS),4) as 'MedSp_BTOS',
...
round( (sum(Nota_TON) / sum(payload)) ,4) as 'MedPd_TON',
round( (sum(Nota_TPH) / sum(payload)) ,4) as 'MedPd_TPH'

from #resumogeologiaeplanejamento

group by PeriodoParametro, OrigemBlock, Hierarchy_Block, Destino, Material_Operator, Material_Block, Material_MES

order by HierarquiaBloco, OrigemBlock